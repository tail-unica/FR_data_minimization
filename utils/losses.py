...
class ElasticArcFace(nn.Module):
    ...
    def forward(self, embbedings, label):
        ...
        margin = torch.normal(mean=self.m, std=self.std, size=label[index, None].size(),
                              device=cos_theta.device)  # Fast converge .clamp(self.m-self.std, self.m+self.std)
        if self.plus:
            with torch.no_grad():
...
class ArcFaceCosNorm(nn.Module):
    def __init__(self, in_features, out_features, s=128.0, m=0.5, init="normal"):
        super(ArcFaceCosNorm, self).__init__()
        self.in_features = in_features
        self.out_features = out_features
        self.s = s
        self.m = m
    ...
    def forward(self, embbedings, label):
        ...
        embbedings_norm = l2_norm(embbedings, axis=1)
        kernel_norm = l2_norm(self.kernel, axis=0)
        ...
        cos_theta.cos_()
        cos_theta.mul_(self.s)
        positive_sims = cos_theta[torch.arange(cos_theta.size(0)).unsqueeze(1), label.view(-1, 1)].std()

        return cos_theta, positive_sims


class CosFaceCosNorm(nn.Module):
    def __init__(self, in_features, out_features, s=64.0, m=0.35):
        super(CosFaceCosNorm, self).__init__()
        self.in_features = in_features
        self.out_features = out_features
        self.s = s
        self.m = m
    ...
    def forward(self, embbedings, label):
        ...
        cos_theta[index] -= m_hot
        ret = cos_theta * self.s
        positive_sims = ret[torch.arange(ret.size(0)).unsqueeze(1), label.view(-1, 1)].std()
        return ret, positive_sims